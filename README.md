...
(`set_selection`) し、`selection` にも出てきます。ただし macOS 以外では、
絞っている表で**画面の外の行を ⌘ / Ctrl で選び足したあと、別の行をふつうに
押すと、外の選択は落ちます** (ネイティブのコントロールへ届くのは、組み立てて
ある行の選択だけのためです)。
...
- 行数が 200 を超える `Table` は、画面に出ている範囲だけを組み立てます
  ...
  同じ**とみなし、必要なら `set_row_height` で決めます。macOS 以外では、
  絞っている表で画面の外の行を ⌘ / Ctrl で選び足したあと、別の行をふつうに
  押すと外の選択は落ちます (ネイティブへ届くのは組み立ててある行の選択だけの
  ためです)。 Web では、行数 × 行の高さがブラウザの扱える要素の高さ
