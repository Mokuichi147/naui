...
//! | Linux | GTK 4.14 / libadwaita 1.5 (Ubuntu 24.04 / Wayland) で `gallery` の全タブ (トースト・折りたたみ・スイッチ・色ピッカー・時刻ピッカーを含む) を実行確認。GTK4 の実コントロールに対する自動テスト 106 件 (スイッチ・色ピッカー・時刻ピッカー・テーブルを含む)。メディアは実ファイル (H.264 + AAC) の再生・シーク・状態変化まで確認 |
